...
const Home = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get(`http://${window.location.hostname}:${PORT}/barchart?` + Math.random())
    ...
    <div>
      <div className="main" style={mainLayout2Style}>
        ...
        <Box title="chart-table" style={{ overflow: 'auto' }}>
          <Charttable data={Array.from({ length: Object(dataCharttable.columnList).length }, (_, i) => ({
        ...
        <Box title="tree-chart" style={{ display: 'flex', alignItems: 'center', overflow: 'auto' }}>
          <Treechart data={[dataTreechart]} />
        </Box>
        <Box title="model" style={{ display: 'flex', alignItems: 'center', overflow: 'auto' }}>
